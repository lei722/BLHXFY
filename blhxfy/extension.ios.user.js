...
(function () {
	...
	const getLocalConfig = () => {
	  ...
	  const keys = ['autoDownload', 'bottomToolbar', 'displayName', 'removeScroller', 'hideSidebar', 'originText', 'storyOnly', 'transJa', 'transEn', 'keepBgm', 'font', 'transApi', 'fontBold', 'plainText', 'battleTrans', 'log'];
	  keys.forEach(key => {
  ...
  `;
	...
	*/
	...
	const getScenario = async name => {
	  ...
	  if (!csv) {
	    if (!scenarioData) {
	      const binaryString = await fetchWithHash('/blhxfy/data/story.json');
